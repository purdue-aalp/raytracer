...
class ValExec_RecFNCompareSelect(
  ...
  val unit = Module(new RecFNCompareSelect(option = option, passthrough_type = Bits(33.W)))
  ...
  when(io.expected.c_out === io.actual.c_out && io.expected.d_out === io.actual.d_out){
    io.pass := true.B
  }.otherwise{
    io.pass := false.B
  }
