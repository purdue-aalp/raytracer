...
class Datapath_test_special_cases extends AnyFreeSpec with ChiselScalatestTester {
  ...
  def testRayBoxIntersection(
    ...
    val sw_result_seq: List[SW_RayBox_Result] = {
      ray_box_list.map {
        case SW_EnhancedCombinedData(r, bseq, t, SW_OpQuadbox, _, _, _, _, _) => {
    ...
    description in {
      ...
      val sw_result_seq: List[SW_RayTriangle_Result] = {
        ray_triangle_list.map {
          case SW_EnhancedCombinedData(r, _, t, SW_OpTriangle, _, _, _, _, _) => {
            // println("calculated a sw result")
